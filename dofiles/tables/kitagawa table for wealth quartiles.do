...
foreach g of numlist 2/5 {
	...
	* get weights and group outcomes
	foreach p of numlist 0/3 {
		...
		eststo wealth`p': reg v201 v201 // dummy regressison for eststo
		
		sum wealth`p' if groups6==1 & preg==1 [aw=v005]
		local fwd_wt_`p' = r(mean)
		
		eststo wealth`p': estadd scalar fwd_wt = r(mean)
		
		sum wealth`p' if groups6==`g' & preg==1 [aw=v005]
		local g_wt_`p' = r(mean)
		eststo wealth`p': estadd scalar g_wt = r(mean)
		
		sum `outcome' if groups6==1 & wealth==`p' & preg==0 [aw=reweightingfxn]
		local fwd_outcome_`p' = r(mean)
		
		if "`outcome'"=="underweight" local fwd_outcome_`p' = r(mean)*100 // rescale to % for underweight
		
		eststo wealth`p': estadd scalar fwd_outcome = `fwd_outcome_`p''
		
		sum `outcome' if groups6==`g' & wealth==`p' & preg==0 [aw=reweightingfxn]
		local g_outcome_`p' = r(mean)
		
		if "`outcome'"=="underweight" local g_outcome_`p' = r(mean)*100 // rescale to % for underweight
		
		eststo wealth`p': estadd scalar g_outcome = `g_outcome_`p''
		
		eststo wealth`p': estadd scalar total_diff = (`g_outcome_`p''-`fwd_outcome_`p'')
		
		local within_group_`p' = (`g_outcome_`p''-`fwd_outcome_`p'')*(`g_wt_`p''+`fwd_wt_`p'')/2
		eststo wealth`p': estadd scalar within_group = `within_group_`p''
		
		local between_group_`p' = (`g_wt_`p''-`fwd_wt_`p'')*(`g_outcome_`p''+`fwd_outcome_`p'')/2
		eststo wealth`p': estadd scalar between_group = `between_group_`p''
	...
	* format and export table
	...
	#delimit ;
	esttab wealth1 wealth2 wealth3 wealth4 total pct,
