...
foreach g of numlist 2/5 {
	...
	* get weights and group outcomes
	foreach p of numlist 1/3 {
		...
		eststo birth_space_cat`p': estadd scalar g_outcome = `g_outcome_`p''
		
		eststo birth_space_cat`p': estadd scalar total_diff = (`g_outcome_`p''-`fwd_outcome_`p'')
		
		local within_group_`p' = (`g_outcome_`p''-`fwd_outcome_`p'')*(`g_wt_`p''+`fwd_wt_`p'')/2
		eststo birth_space_cat`p': estadd scalar within_group = `within_group_`p''
		
		local between_group_`p' = (`g_wt_`p''-`fwd_wt_`p'')*(`g_outcome_`p''+`fwd_outcome_`p'')/2
		eststo birth_space_cat`p': estadd scalar between_group = `between_group_`p''
	...
	* format and export table
	...
	#delimit ;
	esttab bs1 bs2 bs3 bs4 total pct,
	...
	esttab bs1 bs2 bs3 bs4 total pct using "tables/kitagawa_`outcome'_`g'_bs.tex",
		replace
		stats(fwd_wt fwd_outcome g_wt g_outcome total_diff within_group between_group, labels(`labels') fmt(2))
		drop(v201 _cons)
		nonumbers nostar noobs not
