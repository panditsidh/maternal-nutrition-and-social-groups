...
if "`c(username)'" == "dc42724" {
	global out_tex "C:\Users\dc42724\Documents\GitHub\maternal-nutrition-and-social-groups\tables\kitagawa"
...
foreach p of numlist 0/3 {
	...
	eststo parity`p': estadd scalar fwd_outcome = r(mean)
	
	sum `outcome' if groups6==`g' & parity==`p' & preg==0 [aw=reweightingfxn]
	local g_outcome_`p' = r(mean)
	eststo parity`p': estadd scalar g_outcome = r(mean)
...
esttab parity0 parity1 parity2 parity3 total pct using "${out_tex}`g'.tex",
	replace
	stats(fwd_wt fwd_outcome g_wt g_outcome total_diff within_group between_group, labels(`labels') fmt(2))
	drop(v201 _cons)
	nonumbers nostar noobs not
