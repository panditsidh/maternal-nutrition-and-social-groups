...
#delimit ; 
local varlist not_c_user less_edu rural noboy age1519 age2024 age2529 age3049 parity_bs1 parity_bs2 parity_bs3 parity_bs4 parity_bs5 parity_bs6 parity_bs7 parity_bs8 parity_bs9 parity_bs10 wealth1 wealth2 wealth3 wealth4; 
#delimit cr 
...
* --- True unweighted N (no duplication) ---
preserve
...
* --- Now do your duplication for India means ---
expand 2, gen(dup)
...
* display but leave blank if rows are not empty
foreach i of numlist 1/12 {
	gen disp_v`i' = string(v`i') if _n==33
	replace disp_v`i' = substr(string(v`i', "%4.2f"), 2, .) if (rows!="" & strmatch(rows, "\textbf{*")==0) & _n<33
}
