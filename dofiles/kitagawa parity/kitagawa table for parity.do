...
foreach g of numlist 2/5 {
	...
	* format and export table
	...
	#delimit ;
	esttab parity1 parity2 parity3 parity4 total pct,
		...
		mtitles("parity 1" "parity 2" "parity 3" "parity 4+" "Total" "Percent");

	
	esttab parity1 parity2 parity3 parity4 total pct using "tables/kitagawa_`outcome'_`g'_parity.tex",
		replace
		stats(fwd_wt fwd_outcome g_wt g_outcome total_diff within_group between_group, labels(`labels') fmt(2))
		drop(v201 _cons)
		nonumbers nostar noobs not
